...
# Copy over the Maven settings.xml file and make sure all files are owned by the new user
...
# Pull down the service
#RUN 	mkdir /home/jservice/.m2 2>/dev/null && cp /root/.m2/settings.xml /home/jservice/.m2 && \
#        chown -R jservice.jservice /home/jservice
...
RUN     mkdir ~/service && cd ~/service && \
        mvn -U org.apache.maven.plugins:maven-dependency-plugin:2.8:get -DgroupId=${APP_GROUP_ID} -DartifactId=${APP_ARTIFACT_ID} -Dversion=${APP_VERSION} -Dtype=jar -Dtransitive=false && \
